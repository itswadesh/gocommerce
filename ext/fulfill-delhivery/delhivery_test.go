...
func TestInstalledIdleUntilConfigured(t *testing.T) {
	...
	if _, err := app.Ship().Create(ctx, result.Order.ID, "delhivery", gocommerce.ShipRequest{Tracking: "T-1"}); err == nil || !strings.Contains(err.Error(), "not set up") {
		t.Fatalf("shipping through an idle provider = %v, want a refusal that says it is not set up", err)
	}

	on := true
	if _, err := app.Plugins().Update(ctx, PluginKey, gocommerce.PluginPatch{Enabled: &on, Settings: map[string]any{"token": "t-1", "pickup_location": "Primary", "seller_name": "Acme"}}); err != nil {
		t.Fatalf("configure from the panel: %v", err)
	}
	for _, p := range app.Settings().FulfillmentProviders {
		if p.Code == "delhivery" && !p.Configured {
