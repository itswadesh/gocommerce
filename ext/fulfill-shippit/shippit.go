...
func (m *Module) Register(app *gocommerce.App) error {
	...
	case strings.TrimSpace(m.cfg.APIKey) == "":
		return errors.New("shippit: APIKey is required")
	case strings.TrimSpace(m.cfg.CourierType) == "":
		return errors.New("shippit: CourierType is required — the engine will not choose a service band on your behalf")
	}
	if m.cfg.BaseURL == "" {
...
func (m *Module) orderBody(order *gocommerce.Order, req gocommerce.ShipRequest) map[string]any {
	...
	body := map[string]any{
		"courier_type":            firstNonEmpty(req.Meta["courier_type"], m.cfg.CourierType),
...
func (m *Module) parcelBody(req gocommerce.ShipRequest) map[string]any {
	...
	return map[string]any{
		...
		"length": metres(req.Parcel.Dimensions.Length, m.cfg.DefaultLengthMM),
		"width":  metres(req.Parcel.Dimensions.Width, m.cfg.DefaultWidthMM),
		"depth":  metres(req.Parcel.Dimensions.Height, m.cfg.DefaultHeightMM),
	}
}
